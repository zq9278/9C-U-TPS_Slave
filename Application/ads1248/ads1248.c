...
void ADS1248_Init(void)
{
    ...
    // 读回寄存器看看到底是什么
uint8_t regs[16] = {0};
ADS1248_ReadRegs(0x00, regs, 16);

LOG_I("ADS1248 Reg Dump:");
for(int i = 0; i < 16; i++) {
    LOG_I(" reg[%02d] = 0x%02X", i, regs[i]);
}
...
{
    uint8_t cmd[4] = { ADC_CMD_RDATA, ADC_CMD_NOP, ADC_CMD_NOP, ADC_CMD_NOP };
    uint8_t buf[4];

    RTD_CS(0);
    HAL_SPI_TransmitReceive(&hspi2, cmd, buf, 4, 100);
    RTD_CS(1);

    // buf[0] 是 RDATA之后第一字节（高字节）
    int32_t raw = ((int32_t)buf[1] << 16) |
                  ((int32_t)buf[2] << 8)  |
                  (int32_t)buf[3];

    // 24bit 有符号数扩展（如果你确定是单极性，可以忽略）
    if (raw & 0x800000) {
        raw |= 0xFF000000;
    }
...
{
    uint8_t cmd[2];
    cmd[0] = ADC_CMD_WREG | startReg;   // 0x40 | reg
    cmd[1] = (len - 1);                 // 写 len 个寄存器
...
{
    uint8_t cmd[2];
    cmd[0] = ADC_CMD_RREG | startReg;   // 0x20 | reg
    cmd[1] = (len - 1);
...
{
    ADS1248_SendCommand(0xF0);  // 假设 0xF0 为 SELFCAL，具体看 datasheet
    HAL_Delay(5);
}
